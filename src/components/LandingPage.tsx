...
export default function LandingPage({ onGetStarted, onDirectLogin, isLoggedIn }: LandingPageProps) {
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col relative overflow-hidden font-sans selection:bg-indigo-500 selection:text-white">
      ...
      <div className="absolute top-[40%] left-[30%] w-[300px] h-[300px] rounded-full bg-blue-900/10 blur-[100px] pointer-events-none" />
      ...
      <header className="relative z-10 max-w-7xl mx-auto w-full px-6 py-5 flex items-center justify-between border-b border-slate-900">
        ...
        <div className="flex items-center gap-4">
          <span className="hidden sm:inline-block px-3 py-1 text-xs font-semibold text-indigo-400 bg-indigo-950/40 border border-indigo-800/30 rounded-full">
            v2.1 Stable
          </span>
          {isLoggedIn ? (
            <button 
              onClick={onGetStarted}
              className="px-4.5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-xl transition-all shadow-lg shadow-indigo-600/10 flex items-center gap-1.5"
            >
            ...
            <button 
              ...
              className="px-4.5 py-2 bg-slate-900 hover:bg-slate-800 text-slate-200 hover:text-white text-xs font-bold rounded-xl border border-slate-800 transition-all"
            >
              Masuk / Daftar
            </button>
          )}
      ...
      <main className="relative z-10 flex-1 max-w-7xl mx-auto w-full px-6 flex flex-col items-center justify-center text-center py-20 lg:py-28">
        ...
        {/* Sparkle Tagline */}
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-indigo-950/60 border border-indigo-500/30 text-indigo-200 text-xs font-bold mb-6 animate-pulse">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-10 w-full justify-center">
          <button
            onClick={onGetStarted}
            className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 hover:from-indigo-500 hover:to-blue-500 text-white font-bold rounded-xl shadow-xl shadow-indigo-600/20 hover:shadow-indigo-600/30 transition-all flex items-center justify-center gap-2 shrink-0 text-sm md:text-base cursor-pointer"
          >
          ...
          {!isLoggedIn && (
            <button
              onClick={onDirectLogin}
              className="w-full sm:w-auto px-8 py-4 bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white font-bold rounded-xl border border-slate-800 transition-all text-sm md:text-base cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl w-full mt-24">
          ...
          {/* Card 1 */}
          <div className="p-6 bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-slate-900 hover:border-indigo-500/30 transition-all text-left group">
            <div className="h-11 w-11 rounded-xl bg-indigo-950/60 border border-indigo-500/30 text-indigo-400 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
          ...
          {/* Card 2 */}
          <div className="p-6 bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-slate-900 hover:border-emerald-500/30 transition-all text-left group">
            <div className="h-11 w-11 rounded-xl bg-emerald-950/60 border border-emerald-500/30 text-emerald-400 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
            ...
            <p className="text-slate-400 text-sm mt-2.5 leading-relaxed">
              Pantau status siklus akun mulai dari <strong className="text-emerald-400 font-medium">Ready</strong>, <strong className="text-yellow-400 font-medium">Proses</strong> optimasi, hingga <strong className="text-indigo-400 font-medium">Sold</strong> (laku terjual) dengan filter klik cepat.
            </p>
          </div>

          {/* Card 3 */}
          <div className="p-6 bg-slate-900/40 backdrop-blur-sm rounded-2xl border border-slate-900 hover:border-sky-500/30 transition-all text-left group">
            <div className="h-11 w-11 rounded-xl bg-sky-950/60 border border-sky-500/30 text-sky-400 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
        ...
        <div className="mt-16 pt-8 border-t border-slate-900/80 w-full max-w-4xl flex flex-wrap items-center justify-center gap-12 text-center">
          ...
          <div>
            <p className="text-3xl font-black text-indigo-400">Offline</p>
